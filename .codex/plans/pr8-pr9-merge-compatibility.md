...
- 发现：PR8 在 GitHub 上以 squash merge 形成新的 `main` 提交 `e51ccdd`，该提交不是 PR8 分支提交的祖先。
- 影响：即使 PR9 已合入 PR8 分支，GitHub 仍会以旧合并基线判定 PR9 为冲突。
- 处理方式：将 `origin/main` 合并到 PR9，并以已验证的叠加结果解决冲突；不改写历史或强制推送。
