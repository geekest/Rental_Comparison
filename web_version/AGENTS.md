...
## Rental Comparison Visual Direction

- 唯一视觉真值为 `docs/design/references/candidate-gallery.png` 与 `docs/design/references/progressive-comparison.png`。
- 候选页沿用 Apple Store 风格的大标题、横向大卡片、下一张露出和渐进披露。
- 比较页沿用对齐的房源头部、分段主题和逐项纵向比较。
- 不再混入小宇宙方向；不使用播客式列表、青色主色或生活方式插画语言。
- 视觉可以使用中性暖色房源照片，但界面仍保持 Apple 平台的层级、留白和控件行为。
