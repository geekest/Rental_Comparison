...
struct ListingInlineFieldsView: View {
    ...
    var body: some View {
        Group {
            Section("基本信息") {
                TextField("房源名称", text: $listing.name)
                TextField("城市", text: $listing.city)
                Picker("租赁方式", selection: $listing.rentalType) {
                    ForEach(RentalType.allCases) { Text($0.title).tag($0) }
                }
                TextField("月租", value: $listing.rent, format: .number).keyboardType(.decimalPad)
                TextField("货币", text: $listing.currency).textInputAutocapitalization(.characters)
                TextField("居室数", value: optionalIntBinding(\.roomCount, fallback: 1), format: .number).keyboardType(.numberPad)
            }
            ...
            Section("位置与空间") {
                TextField("地址", text: optionalStringBinding(\.address))
                TextField("面积", value: $listing.area, format: .number).keyboardType(.decimalPad)
                Picker("面积范围", selection: optionalStringBinding(\.areaScope, fallback: "整套")) {
                    Text("整套").tag("整套")
                    Text("私人空间").tag("私人空间")
                }
                TextField("户型", text: optionalStringBinding(\.layout))
                TextField("楼层", text: optionalStringBinding(\.floor))
                Picker("电梯", selection: $listing.hasElevator) {
                    Text("待补充").tag(Bool?.none)
                    Text("有").tag(Bool?.some(true))
                    Text("无").tag(Bool?.some(false))
                }
                TextField("租期（月）", value: $listing.leaseMonths, format: .number).keyboardType(.numberPad)
            }
            ...
            Section("通勤") {
                ...
                TextField("单程分钟", value: $listing.commuteMinutes, format: .number).keyboardType(.numberPad)
                TextField("单次支出", value: $listing.commuteFare, format: .number).keyboardType(.decimalPad)
            }
            ...
            Section("费用明细") {
                ForEach($listing.costs) { $item in
                    DisclosureGroup(item.name.isEmpty ? "未命名费用" : item.name) {
                        ...
                        TextField("金额", value: $item.amount, format: .number).keyboardType(.decimalPad)
