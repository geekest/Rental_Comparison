...
- 状态：已实现，真机与在线平台验收中
- 最近更新：2026-08-24
- 当前基线：`codex/listing-import-share-extension`，提交 `93233e5`
- 代码验证：Simulator 构建、解析 fixture、XCTest 和关键 XCUITest 已覆盖；真实系统分享面板、相册链路和在线详情页不视为已验证。
...
7. 保存后，用户可在房源详情顶部查看封面、导入图片和原始截图；可添加图片、移除单张图片，并选择任意一张作为主图。
...
- 主图保存为 Option 的可选 Evidence ID；卡片与详情顶部优先使用主图。缺失主图的旧数据按既有媒体顺序显示。
- 原始截图和用户添加照片都属于房源媒体；看房验证任务的现场照片不进入房源图片管理。
